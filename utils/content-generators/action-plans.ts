...
export interface ActionPlanRules {
  renter: { priority: string; action: string; costImpact: string };
  homeowner: { priority: string; action: string; systemType: string };
  commercial: { complianceRisk: string; requirement: string };
}

export function generateCityActionPlan(cityName: string, hardness: number): ActionPlanRules {
  // Return strict diagnostic rules based on EXACT mg/L, not spun strings.
  // This provides actual utility to the user based on raw physics.
  
  const isCritical = hardness > 180;
  const isHard = hardness > 120;
  
  return {
    renter: {
      priority: isHard ? "High" : "Low",
      action: isHard ? "Install point-of-use filtration (Shower/Kitchen)" : "Standard appliance maintenance",
      costImpact: isHard ? "Est. €40-60/yr in descaling agents" : "Minimal routine costs"
    },
    homeowner: {
      priority: isCritical ? "Urgent" : isHard ? "Recommended" : "Optional",
      action: isCritical ? "Install whole-house ion exchange" : isHard ? "Install salt-free conditioner or ion exchange" : "Carbon filtration for taste preferences",
      systemType: isCritical ? "Twin-tank softener system" : isHard ? "Single-tank softener system" : "Under-sink RO or Carbon filter"
    },
    commercial: {
      complianceRisk: isCritical ? "Severe equipment failure risk" : isHard ? "Moderate scaling risk" : "Low risk",
      requirement: isHard ? "Commercial Calcium Treatment Unit (CTU) mandatory for equipment warranties" : "Standard sediment/carbon filtration"
    }
  };
}