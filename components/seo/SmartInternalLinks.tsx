...
export function SmartInternalLinks({ 
  ...
  // MAJOR HUBS FOR CRAWL BRIDGES (Prevents isolated Regional Silos)
  const majorHubs = [
    { name: 'London', url: `/${lang}/united-kingdom/greater-london/london` },
    { name: 'Berlin', url: `/${lang}/germany/berlin/berlin` },
    { name: 'Paris', url: `/${lang}/france/ile-de-france/paris` },
  ...
  // Filter out the current city and limit to 6 to prevent link bloat
  const validSiblings = siblingCities
    .filter(sibling => sibling.slug !== citySlug)
    .slice(0, 6);
  ...
  return (
    <div className="mt-16 border-t border-slate-200 pt-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-xl font-bold text-slate-900 m-0">Explore Nearby Municipalities</h3>
      </div>
      
      {/* Sibling Cities Grid - Consistent Anchor Text */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {validSiblings.map((sibling) => (
          <Link 
            key={`${sibling.region_slug}-${sibling.slug}`}
            href={`/${lang}/${sibling.country_slug}/${sibling.region_slug}/${sibling.slug}`}
            className="flex items-center gap-2 p-3 rounded-lg hover:bg-blue-50 text-blue-700 hover:text-blue-900 transition-colors border border-transparent hover:border-blue-100 text-sm font-medium"
          >
            <MapPin className="w-4 h-4 text-blue-400 shrink-0" />
            <span className="truncate">{sibling.name} Water Data</span>
          </Link>
        ))}
      </div>

      {/* Footer Links & Crawl Bridge */}
      <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        
        <Link 
          href={`/${lang}/${countrySlug}/${regionSlug}`} 
        ...
        <Link 
          href={randomHub.url} 
          className="text-sm text-slate-600 hover:text-slate-900 font-medium flex items-center gap-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-4 py-2 rounded-full transition-colors"
        >
           <Globe className="w-4 h-4 text-slate-400" /> 
           Compare with {randomHub.name}
        </Link>
