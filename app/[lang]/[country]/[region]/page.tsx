...
export default async function RegionHubPage({ params }: PageProps) {
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 md:p-12 text-zinc-800 font-sans">
      <header className="mb-16 border-b border-zinc-200 pb-10">
        <Breadcrumbs items={breadcrumbItems} />
        <h1 className="text-4xl md:text-5xl font-semibold text-zinc-900 tracking-tighter mt-6 mb-5">
          {t.water_hardness_in} {displayRegion}
        </h1>
      ...
      {/* The Region Directory Grid */}
      <section className="bg-white p-8 md:p-10 rounded-[2rem] border border-zinc-200 shadow-sm mb-20">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8 border-b border-zinc-100 pb-5">
        ...
        <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
          {sortedCities.map(city => (
            <li key={city.id}>
              <Link 
                href={`/${lang}/${decodedCountry}/${decodedRegion}/${city.name.toLowerCase()}`}
                className="group flex flex-col p-5 rounded-2xl border border-zinc-100 hover:border-sky-200 hover:bg-sky-50/50 hover:shadow-lg hover:shadow-zinc-100/50 transition-all duration-300"
              >
                <span className="font-medium text-zinc-800 group-hover:text-sky-800 transition-colors">
                  {city.name}
                </span>
                <span className="text-sm text-zinc-500 mt-2 flex justify-between items-center">
                  <span className="tabular-nums font-light">{city.hardness_mg_l} mg/L</span>
                  <span className="text-sky-500 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-300">&rarr;</span>
                </span>
              </Link>
            </li>
          ))}
